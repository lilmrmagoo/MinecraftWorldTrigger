...
public class MCreatorFormAndBulletswap extends Elementsworld_trigger_craft.ModElement {
	private KeyBinding keys;
	public MCreatorFormAndBulletswap(Elementsworld_trigger_craft instance) {
		super(instance, 54);
	}
	...
	public void init(FMLInitializationEvent event) {
		keys = new KeyBinding("key.mcreator.formandbulletswap", Keyboard.KEY_V, "key.categories.misc");
	...
	public void onKeyInput(InputEvent.KeyInputEvent event) {
		if (Minecraft.getMinecraft().currentScreen == null) {
			if (org.lwjgl.input.Keyboard.isKeyDown(keys.getKeyCode())) {
				world_trigger_craft.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage());
	...
	private static void pressAction(EntityPlayer entity) {
			...
			return;
		{
			...
			MCreatorFormAndBulletswapOnKeyPressed.executeProcedure($_dependencies);
		}
	}
}
